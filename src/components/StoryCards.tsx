...
export default function StoryCards({ lang, setLang, data, stateCode, onReset, onOpenInfo }: StoryCardsProps) {
  ...
  const handleShare = async () => {
    const textToShare = lang === "en"
      ? `🎓 My School Wrapped ${new Date().getFullYear()} (${stateInfo.name})!
📊 My report:
⏰ Lesson Hours: ${lessons.toLocaleString()} in the classroom
📝 Homework: ~${homework} solved tasks (estimate)
🏆 Top Subjects: ${topThree.length > 0 ? topThree.map(t => `${t.subject} (${t.grade})`).join(", ") : "No grades extracted"}
🚀 Attendance: ${attendanceRate}% ${hasDaysInfo ? `(${schoolDays - totalAbsentDays} of ${schoolDays} days present)` : ""}${hasLessonsInfo ? ` with ${totalAbsentLessons} missed lessons` : ""}

Calculate your own School Wrapped at ${window.location.origin} 🎒🔥`
      : `🎓 Mein Schulzeit Wrapped ${new Date().getFullYear()} (${stateInfo.name})!
📊 Meine Bilanz:
...
🚀 Anwesenheit: ${attendanceRate}% ${hasDaysInfo ? `(${schoolDays - totalAbsentDays} von ${schoolDays} Tagen da gewesen!)` : ""}${hasLessonsInfo ? ` mit ${totalAbsentLessons} Fehlstunden` : ""}
              ...
              <div className="space-y-4">
                <div className="text-center mb-3">
                  ...
                  <p className="text-[10px] text-slate-400 mt-1 font-mono uppercase tracking-wider">
                    {t.card5_summary} • {stateInfo.name}
                  </p>
                </div>
